...
fn main() {
    let mut io = IO::new();
    let (m, n, k, s): (usize, usize, usize, f32) = io.r4();
    let mut arr= [f32::MAX; MAX];
    let mut i = 0;
    for _ in 0..2 {
        for x in io.line() {
            arr[i] = x;
    ...
    let n = m+n;
    arr[..n].sort_by(|a, b| a.partial_cmp(b).unwrap());
    let mut ap = [[f32::MAX; MAX]; MAX];
    let mut dp = [[f32::MAX; MAX]; 101];
    for i in 0..n {
        let mut tot = 0.0;
        for j in i..n {
            tot += arr[j];
            let a = tot / (j-i+1) as f32;
            let mut d = 0.0;
            for l in i..j+1 {
                d += (arr[l]-a).powi(2);
            }
    ...
    for l in 2..k+1 {
        for i in l-1..n {
            for j in i..n {
                dp[l][j] = dp[l][j].min(dp[l-1][i-1] + ap[i][j]); 
            }
        }
    }
    println!("{}", dp[k][n-1] + n as f32 * (s/2.0).powi(2));
}
