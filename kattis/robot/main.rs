...
fn main() {
    ...
    let mut points = [Vector{x: 0, y: 0}; 10000];
    loop {
        let _ = stdin.read_line(&mut buf);
        let trimmed_buf = buf.trim();
        let n: usize = trimmed_buf.parse().unwrap();
        if n == 0 { return }
        buf.clear();
        for i in 0..n {
            let _ = stdin.read_line(&mut buf);
        ...
        let hull = graham_scan(&mut points[..n]);
        println!("{}", shoelace(&hull));
    }
}
...
fn graham_scan(points: &mut [Vector]) -> Vec<Vector> {
    ...
    for p in &points[1..] {
        while hull.len() > 1 && orient(&hull[hull.len()-2], &hull[hull.len()-1], p) > 0{
            hull.pop();
        }
        hull.push(*p);
    }
...
fn shoelace(hull: &Vec<Vector>) -> f32 {
    let mut left = 0;
    let mut right = 0;
    let n = hull.len();
    for i in 0..n {
        let j = (i+1) % n;
        left += hull[i].x*hull[j].y;
        right += hull[i].y*hull[j].x;
    }
    return ((left-right).abs() as f32) / 2.0;
}
